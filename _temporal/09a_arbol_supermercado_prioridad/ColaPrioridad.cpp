...
// Encola: inserta al final y sube hasta su posición correcta (bubble up)
void ColaPrioridad::encolar(int nuevoElemento)
{
	...
	while (pos > 0)
	{
		...
		{
			break; // Ya está en su sitio
		}
	}
}

// Desencola: devuelve la raíz (mínimo), coloca el último en la raíz y reestructura
int ColaPrioridad::desencolar()
{
	int raiz = vector.getValor(0);

	// Ponemos el último elemento en la raíz
...
// Reestructura el montículo desde la raíz hacia abajo (sift down)
